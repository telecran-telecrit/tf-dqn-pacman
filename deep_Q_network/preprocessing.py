...
#WALL_COLOR_GRAY = cv2.cvtColor(np.array([[WALL_COLOR]], dtype=np.uint8), cv2.COLOR_RGB2GRAY)[0][0]
#BACKGROUND_GRAY = cv2.cvtColor(np.array([[BACKGROUND]], dtype=np.uint8), cv2.COLOR_RGB2GRAY)[0][0]
#PACMAN_COLOR_GRAY = cv2.cvtColor(np.array([[PACMAN_COLOR]], dtype=np.uint8), cv2.COLOR_RGB2GRAY)[0][0]
...
def preprocess_observation(observations: List[np.array], new_obs: np.array):
    ...
    state = np.concatenate(observations) # state = np.concatenate(observations);  screen = torch.from_numpy(state)
    return img_to_array(state) # screen.unsqueeze(0)
